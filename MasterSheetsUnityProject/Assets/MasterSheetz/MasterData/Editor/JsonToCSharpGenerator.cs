...
namespace App.MasterData.AutoGenerated
{
    internal static class JsonToCSharpGenerator
    {
        ...
        private static void EnsureOutputDirectoryExists(string outputFolderPath)
        {
            ...
            {
                ShowErrorDialog("Error", "Output folder path is not set.");
                return;
            }
        ...
        private static void CleanOutputDirectory(string outputFolderPath)
        {
            ...
            {
                ShowErrorDialog("Error", $"Failed to clean output directory: {ex.Message}");
            }
        }
        ...
        private static string[] GetJsonFiles(string folderPath)
        {
            ...
            {
                ShowErrorDialog("Json File Error", $"The specified folder does not exist: {folderPath}");
            }
        ...
        private static void ProcessJsonFile(string jsonFilePath, string outputFolderPath, string nameSpace)
        {
            try
            {
                ...
                var generatedClass =
                    GenerateCSharpClass(jsonRoot.fileName, jsonRoot.columns, jsonRoot.types, nameSpace);
                WriteGeneratedClassToFile(generatedClass, jsonRoot.fileName, outputFolderPath);
            }
            catch (Exception ex)
            {
                ShowErrorDialog("Make Error", ex.Message);
            }
        }

        private static bool IsValidJsonRoot(RootJson jsonRoot)
        {
            return jsonRoot != null && jsonRoot.fileName != null && jsonRoot.types != null && jsonRoot.columns != null;
        }
            ...
            string nameSpace)
        {
            ...
            classDef.AppendLine("using MasterData.Runtime.Domain;");
            classDef.AppendLine();
            classDef.AppendLine($"namespace {nameSpace}");
            classDef.AppendLine("{");
            classDef.AppendLine("    [Serializable]");
            classDef.AppendLine($"    public class {className} : IMasterData");
            classDef.AppendLine("    {");
            ...
            classDef.AppendLine($"    public partial class {className}MasterDataTable : MasterDataTable<{className}>");
            classDef.AppendLine("    {");
            classDef.AppendLine(
                $"        public {className}MasterDataTable(IEnumerable<{className}> masterData) : base(masterData) {{ }}"
            );
            classDef.AppendLine("    }");
            classDef.AppendLine("}");
